function [ cell_lines, cell_slope ] = linesDetect( cell_paths, cell_imgs, C1, C2, ref_n )
...
num_imgs = size(cell_imgs,1);
cell_lines = cell(num_imgs,1);
cell_slope = cell(num_imgs,1);
for i=1:num_imgs
    if i==ref_n;  continue;  end  % do not need detect lines in reference 
    lines = lsd(cell_paths{i});
    img = cell_imgs{i};
    [M, N, ~] = size(img);
...
%     hold off
    ...
    if isempty(sample_lines); sample_lines=[0,1;0,1]; end
    cell_lines{i} = sample_lines;
    cell_slope{i} = slope_lines;
end
